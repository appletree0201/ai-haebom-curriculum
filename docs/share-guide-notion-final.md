---
title: AI 해봄 컬리큘럼
---

# AI 해봄 학습자료 안내

> **이 페이지의 역할**
>
> 이 페이지는 AI 해봄 학습자료의 입구입니다.
> Notion에서는 전체 흐름을 보고, 실제 학습은 HTML 화면에서 진행합니다.

---

## 먼저 이렇게 보면 됩니다

| 순서  |     볼 자료      |        역할        |
| :-: | :-----------: | :--------------: |
|  1  | Notion 안내 페이지 | 전체 흐름과 회차별 링크 확인 |
|  2  |  HTML 시작 페이지  |     실제 학습 화면     |
|  3  |  설명자료와 시연내용   |   개념 확인과 따라 하기   |
|  4  |  Markdown 원문  |    필요할 때만 참고     |

> **권장 흐름**
>
> Notion에서 회차를 고른 뒤, `HTML 상세보기`를 눌러 학습을 시작합니다.
> HTML 화면 안에서 `설명자료 열기`, `시연내용 열기`, `가이드 보기` 버튼을 눌러 내용을 확인합니다.

---

## 전체 자료

아래 링크는 전체 회차 자료를 한 화면에서 볼 수 있는 시작 페이지입니다.

[전체 HTML 자료 열기](https://appletree0201.github.io/ai-haebom-curriculum/)

처음 참여하는 사람은 전체 페이지를 열고 1회차부터 차례대로 보는 것을 권장합니다.

[모임 참석 확인 운영도구 열기](https://appletree0201.github.io/ai-haebom-curriculum/operations/attendance-check/)

모임 전에 참석 여부, 음료 선택, 요청사항을 가볍게 정리할 때 사용합니다.

[모임 참석 확인 구현 및 테스트 계획](https://appletree0201.github.io/ai-haebom-curriculum/operations/attendance-check/implementation-plan.md)

회원 응답을 서버에 저장하고 운영진 대시보드에서 취합하는 다음 단계를 검토할 때 사용합니다.

---

## 1~10회차 세부 커리큘럼

|  회차  |          주제           |  상태   |                                              자료                                              | 일시 및 장소 |
| :--: | :-------------------: | :---: | :------------------------------------------------------------------------------------------: | :-----: |
| 1회차  |   VS Code와 Codex 시작   | 기본 준비 | [HTML 상세보기](https://appletree0201.github.io/ai-haebom-curriculum/session-01-start-page.html) |         |
| 2회차  | AGENTS.md와 skills 이해  | 작업 규칙 | [HTML 상세보기](https://appletree0201.github.io/ai-haebom-curriculum/session-02-start-page.html) |         |
| 3회차  |   큰 요청을 작은 계획으로 나누기   | 계획 수립 | [HTML 상세보기](https://appletree0201.github.io/ai-haebom-curriculum/session-03-start-page.html) |         |
| 4회차  |    실제 업무자동화 사례 보기     | 사례 공유 | [HTML 상세보기](https://appletree0201.github.io/ai-haebom-curriculum/session-04-start-page.html) |         |
| 5회차  |  자동화 과제 정의와 Markdown  | 과제 정의 | [HTML 상세보기](https://appletree0201.github.io/ai-haebom-curriculum/session-05-start-page.html) |         |
| 6회차  | Windows Git 설치와 기본 기록 | 변경 기록 | [HTML 상세보기](https://appletree0201.github.io/ai-haebom-curriculum/session-06-start-page.html) |         |
| 7회차  |  Obsidian과 LLM Wiki   | 지식 축적 | [HTML 상세보기](https://appletree0201.github.io/ai-haebom-curriculum/session-07-start-page.html) |         |
| 8회차  |  진행 상황 공유와 지원·협업 연결   | 협업 연결 | [HTML 상세보기](https://appletree0201.github.io/ai-haebom-curriculum/session-08-start-page.html) |         |
| 9회차  |     최신 AI 트렌드 공유      | 미정 | [HTML 상세보기](https://appletree0201.github.io/ai-haebom-curriculum/session-09-start-page.html) |         |
| 10회차 |     업무자동화 시작점 공유회     | 확장 시작 | [HTML 상세보기](https://appletree0201.github.io/ai-haebom-curriculum/session-10-start-page.html) |         |

> 9회차는 회차 직전에 가장 화제가 되는 AI 주제를 정합니다.
> 10회차는 끝이 아니라 각자의 업무자동화가 실제 업무 안에서 자라나는 시작점입니다.

---

## 집에서 실습하는 방법

모임 시간에는 시연, 설명, 질문 답변, 토론을 중심으로 합니다.
실습은 각자 집에서 자료를 보며 진행합니다.

1. 해당 회차의 HTML 시작 페이지를 엽니다.
2. 설명자료를 먼저 읽습니다.
3. 시연내용이나 가이드를 같이 봅니다.
4. 본인 PC에서 작게 따라 해 봅니다.
5. 막힌 지점을 기록합니다.
6. 다음 모임이나 동호회 채널에서 질문합니다.

> **중요한 기준**
>
> 한 번에 크게 만들려고 하지 않습니다.
> 작게 요청하고, 결과를 확인하고, 조금 수정하는 방식으로 진행합니다.

---

## 막혔을 때 질문하는 방법

막혔을 때는 “안 됩니다”라고만 말하면 도와주기 어렵습니다.
아래 형식으로 질문하면 원인을 더 빨리 찾을 수 있습니다.

```text
어느 회차 자료를 보고 있었는지:
어느 단계에서 막혔는지:
화면에 나온 문구:
내가 입력한 요청:
원하는 결과:
첨부할 수 있는 화면이나 파일명:
```

예시는 다음과 같습니다.

```text
어느 회차 자료를 보고 있었는지: 6회차 Git 자료
어느 단계에서 막혔는지: git commit 실행
화면에 나온 문구: user.name 설정이 필요하다는 문구
내가 입력한 요청: git commit -m "첫 Git 연습 기록"
원하는 결과: 커밋이 정상적으로 남는 것
첨부할 수 있는 화면이나 파일명: git-practice.md
```

---

## 자료 사용 시 주의사항

> **공유 전 확인**
>
> 이 자료는 학습용 공유 자료입니다.
> 내부자료, 개인정보, 민감정보, 내부 의사결정 내용은 공개 자료에 넣지 않습니다.

- 내부자료를 외부 서비스에 무단 업로드하지 않습니다.
- 개인정보, 민감정보, 내부 의사결정 내용은 공유 자료에 넣지 않습니다.
- AI가 만든 결과는 초안으로 보고 담당자가 확인합니다.
- 실제 업무에 적용하기 전에는 결과가 맞는지 사람이 검토합니다.
- 원본 자료는 수정하지 않고 별도 폴더에 보관합니다.

---
